...
class Laporanaset extends CI_Controller
{
	...
	{
		parent::__construct();
		$this->load->model('Lapaset_model', 'lapasetm');
	 ...
	 public function index()
  {
  	$data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')]) -> row_array();
        ...
        if ($this->form_validation->run() == false) {
			...
			$this->load->view('user/laporan_aset', $data);
            ...
            $query = '';
            if ($table == 'aset_masuk') {
                $query = $this->lapasetm->getAsetMasuk(null, null, ['mulai' => $mulai, 'akhir' => $akhir]);
            } else {
                $query = $this->lapasetm->getAsetKeluar(null, null, ['mulai' => $mulai, 'akhir' => $akhir]);
            }
  ...
  private function _cetak($data, $table_, $tanggal)
    {
      ...
      public function createPdf(){
        ...
        $data['getaset'] = $this->lapasetm->getAset();
        
        $this->load->library('pdf');
        $this->pdf->load_view('user/lapbulaset',$data);
