...
class C_loglaporan extends CI_Controller
{
	 ...
	 public function index()
  {
  	$data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')]) -> row_array();
		...
		$this->load->library('form_validation');
     	...
     	$this->form_validation->set_rules('transaksi', 'Transaksi', 'required|in_list[brgmsk_log,brgklr_log]',[
            'required' => 'Kolom Laporan Transaksi wajib diisi']);
        $this->form_validation->set_rules('tanggal', 'Periode Tanggal', 'required', [
            'required' => 'Kolom Periode Tanggal wajib diisi']);

        if ($this->form_validation->run() == false) {
            $data['title'] = "Laporan Transaksi (Logger)";
            
			$this->load->view('logger/laporan', $data);
            ...
            $query = '';
            if ($table == 'brgmsk_log') {
                $query = $this->logm->getBarangMasuk(null, null, ['mulai' => $mulai, 'akhir' => $akhir]);
            } else {
                $query = $this->logm->getBarangKeluar(null, null, ['mulai' => $mulai, 'akhir' => $akhir]);
            }
  ...
  private function _cetak($data, $table_, $tanggal)
    {
        ...
        $table = $table_ == 'brgmsk_log' ? 'Barang Masuk' : 'Barang Keluar';
      ...
      public function createPdf(){
        ...
        $data['getbarang'] = $this->logm->getBarang();
        
        $this->load->library('pdf');
        $this->pdf->load_view('logger/lapbullog',$data);
