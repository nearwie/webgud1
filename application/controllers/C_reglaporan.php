...
class C_reglaporan extends CI_Controller
{
	 ...
	 public function index()
  {
  	$data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')]) -> row_array();
		...
		$this->load->library('form_validation');
     	...
     	$this->form_validation->set_rules('transaksi', 'Transaksi', 'required|in_list[brg_msk,brg_klr]',[
            'required' => 'Kolom Laporan Transaksi wajib diisi']);
        $this->form_validation->set_rules('tanggal', 'Periode Tanggal', 'required', [
            'required' => 'Kolom Periode Tanggal wajib diisi']);

        if ($this->form_validation->run() == false) {
            $data['title'] = "Laporan Transaksi (Regulator)";
            
			$this->load->view('regulator/laporan', $data);
            ...
            $query = '';
            if ($table == 'brg_msk') {
                $query = $this->regulm->getBarangMasuk(null, null, ['mulai' => $mulai, 'akhir' => $akhir]);
            } else {
                $query = $this->regulm->getBarangKeluar(null, null, ['mulai' => $mulai, 'akhir' => $akhir]);
            }
  ...
  private function _cetak($data, $table_, $tanggal)
    {
        ...
        $table = $table_ == 'brg_msk' ? 'Barang Masuk' : 'Barang Keluar';
      ...
      public function createPdf(){
        ...
        $data['getbarang'] = $this->regulm->getBarang();
        
        $this->load->library('pdf');
        $this->pdf->load_view('regulator/lapbulreg',$data);
