...
class  C_logmasuk extends CI_Controller
{
    ...
    {   
        ...
        $this->load->library('session');
         $data['barangmasuk'] = $this->logm->getBarangMasuk();
         
        
       
        $this->load->view('logger/brg_msk', $data);
    ...
    public function add_stokmsk()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $kode = 'T-BM-' . date('ymd');
            $kode_terakhir = $this->logm->getMax('brgmsk_log', 'id_barang_masuk', $kode);
            ...
            $this->load->view('logger/add_brgmsk', $data);
            
        } else {
            $input = $this->input->post(null, true);
            $insert = $this->logm->insert('brgmsk_log', $input);
    ...
    {   $this->load->model('logger_model');
        ...
        redirect( base_url() . 'C_logmasuk'); 
        }
                
        $a = $this->logm->get_brgmsk_by_id($id);
        
        $this->logm->delete_brgmsk($id);   
