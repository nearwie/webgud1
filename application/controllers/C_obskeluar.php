...
class  C_obskeluar extends CI_Controller
{
    ...
    {   
        ...
        $this->load->library('session');
         $data['barangkeluar'] = $this->obsm->getBarangKeluar();
         
        
       
        $this->load->view('phobs/brg_klr', $data);
    ...
    public function add_stokklr()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['barang'] = $this->obsm->get('master_phujan', null);
              $data['posid'] = $this->obsm->get('tbl_poshujan', null);
             $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')]) -> row_array();

            // Mendapatkan dan men-generate kode transaksi barang masuk
            $kode = 'T-BK-' . date('ymd');
            $kode_terakhir = $this->obsm->getMax('brgklr_phujan', 'id_barang_keluar', $kode);
            ...
            $this->load->view('phobs/add_brgklr', $data);
            
        } else {
            $input = $this->input->post(null, true);
            $insert = $this->obsm->insert('brgklr_phujan', $input);

            if ($insert) {
                set_pesan('data berhasil disimpan.');
                redirect('C_obskeluar');
            } else {
                set_pesan('Opps ada kesalahan!');
                redirect('phobs/add_brgklr');
            }
        }
    }
    ...
    {   $this->load->model('phobs_model');
        ...
        redirect( base_url() . 'C_obskeluar'); 
        }
                
        $a = $this->obsm->get_brgklr_by_id($id);
        
        $this->obsm->delete_brgklr($id);   
