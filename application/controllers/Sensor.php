...
class Sensor extends CI_Controller
{
	...
	public function index ()
	{
        ...
        $data['barangs'] =  $this->senm->getBarang();
		
	
		$this->load->view('sensor/data', $data);
	...
	public function tambah()
    {
        ...
        if ($this->form_validation->run() === FALSE)
        {
            ...
            $kode_terakhir = $this->senm->getMax('master_sensor', 'kode_brg');
            $kode_tambah = substr($kode_terakhir, -4, 4);
            $kode_tambah++;
            $number = str_pad($kode_tambah, 4, '0', STR_PAD_LEFT);
            $data['kode_brg'] = 'SEN' . $number;
        ...
        else
        {
            ...
            $insert = $this->senm->insert('master_sensor', $input);
    ...
    public function edit($getId)
    {
        ...
        if ($this->form_validation->run() === FALSE)
        { 
            $data['barang'] = $this->senm->get('master_sensor', ['kode_brg' => $id]);
        ...
        {
            $input = $this->input->post(null, true);
            $update = $this->senm->update('master_sensor', 'kode_brg', $id, $input);
    ...
    {   $this->load->model('sensor_model');
        ...
        $a = $this->senm->get_brg_by_id($id);
        
        $this->senm->delete_brg($id);   
    ...
    {
        $barang = $this->db->get_where('master_sensor', ['kode_brg' => $kode_brg])->row();
...
public function get_barang(){
    ...
    public function barcode(){
        ...
        $data['btr'] = $this->senm->get_barcode_by_id($id);
    
        $this->load->view('sensor/bc_data', $data);
        ...
        $this->pdf->load_barcode('sensor/bc_cetak',$data);
