...
class Phobs extends CI_Controller
{
	...
	{
		$data['title'] = 'Data Penakar Hujan OBS';
        ...
        $data['barangs'] =  $this->obsm->getBarang();
		
	
		$this->load->view('phobs/data', $data);
	...
	public function tambah()
    {
        ...
        if ($this->form_validation->run() === FALSE)
        {
            ...
            $kode_terakhir = $this->obsm->getMax('master_phujan', 'kode_brg');
            $kode_tambah = substr($kode_terakhir, -4, 4);
            $kode_tambah++;
            $number = str_pad($kode_tambah, 4, '0', STR_PAD_LEFT);
            $data['kode_brg'] = 'OBS' . $number;

          
			$this->load->view('phobs/add', $data);
        ...
        else
        {
            ...
            $insert = $this->obsm->insert('master_phujan', $input);
    ...
    public function edit($getId)
    {
        ...
        if ($this->form_validation->run() === FALSE)
        { 
            $data['barang'] = $this->obsm->get('master_phujan', ['kode_brg' => $id]);
           
			$this->load->view('phobs/edit', $data);
        }
        else
        {
            $input = $this->input->post(null, true);
            $update = $this->obsm->update('master_phujan', 'kode_brg', $id, $input);
    ...
    {   $this->load->model('phobs_model');
        ...
        redirect( base_url() . 'phobs'); 
        }
        $a = $this->obsm->get_brg_by_id($id);
        
        $this->obsm->delete_brg($id);   
    ...
    {
        $barang = $this->db->get_where('master_phujan', ['kode_brg' => $kode_brg])->row();
...
public function get_barang(){
    ...
    public function barcode(){
        ...
        $data['btr'] = $this->obsm->get_barcode_by_id($id);
    
        $this->load->view('phobs/bc_data', $data);
        ...
        $this->load->model(array('phobs_model'));

        
        $data['btr'] = $this->obsm->get_barcode_by_id($id);
        
        $this->load->library('pdf');
        $this->pdf->load_barcode('phobs/bc_cetak',$data);
